...
const XRP_THRESHOLD = 0.01; // Minimum profit threshold for executing trades
const TRADE_AMOUNT = 100; // Amount to trade in XRP

const checkArbitrageOpportunity = async () => {
    try {
        const gasFeeData = await gasFees.findCheapestGasFee('XRP');
        const exchangeData = await exchanges.getExchangeData('XRP'); // Fetch current prices from all exchanges
        ...
        for (const buyExchange of exchangeData) {
            for (const sellExchange of exchangeData) {
                ...
                if (potentialProfit > highestProfit && potentialProfit > XRP_THRESHOLD) {
...
const executeTrade = async (buyExchange, sellExchange, amount) => {
    try {
        ...
        console.log(`Buying ${amount} XRP from ${buyExchange.name}`);
        ...
        console.log(`Selling ${amount} XRP to ${sellExchange.name}`);
