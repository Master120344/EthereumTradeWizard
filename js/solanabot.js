...
const SOL_THRESHOLD = 0.01; // Minimum profit threshold for executing trades
const TRADE_AMOUNT = 10; // Amount to trade in SOL

const checkArbitrageOpportunity = async () => {
    try {
        const gasFeeData = await gasFees.findCheapestGasFee('SOL');
        const exchangeData = await exchanges.getExchangeData('SOL'); // Fetch current prices from all exchanges
        ...
        for (const buyExchange of exchangeData) {
            for (const sellExchange of exchangeData) {
                ...
                if (potentialProfit > highestProfit && potentialProfit > SOL_THRESHOLD) {
...
const executeTrade = async (buyExchange, sellExchange, amount) => {
    try {
        ...
        console.log(`Buying ${amount} SOL from ${buyExchange.name}`);
        ...
        console.log(`Selling ${amount} SOL to ${sellExchange.name}`);
