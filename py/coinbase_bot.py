...
class CoinbaseBot:
    def __init__(self, symbol, reporting_bot_url, email_config, check_interval=5):
        self.api_url = f"https://api.pro.coinbase.com/products/{symbol}/ticker"
        self.symbol = symbol
    ...
    def start(self):
        logging.info(f"Starting CoinbaseBot for {self.symbol}")
    ...
    def stop(self):
        self.stop_event.set()
        self.thread.join()
        logging.info("CoinbaseBot stopped.")
...
if __name__ == "__main__":
    ...
    symbol = "BTC-USD"  # Example trading pair; adjust as needed

    coinbase_bot = CoinbaseBot(symbol, reporting_bot_url, email_config)
    try:
        coinbase_bot.start()
