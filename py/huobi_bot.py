...
class HuobiBot:
    def __init__(self, symbol, reporting_bot_url, email_config, check_interval=5):
        self.api_url = f"https://api.huobi.pro/market/trade?symbol={symbol}"
        self.symbol = symbol
    ...
    def fetch_price(self):
        try:
            ...
            return float(response.json()['data'][0]['price'])  # Last price from the response
    ...
    def start(self):
        logging.info(f"Starting HuobiBot for {self.symbol}")
...
if __name__ == "__main__":
    ...
    huobi_bot = HuobiBot(symbol, reporting_bot_url, email_config)
    try:
        huobi_bot.start()
