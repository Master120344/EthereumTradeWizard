...
class BittrexBot:
    def __init__(self, symbol, reporting_bot_url, email_config, check_interval=5):
        self.api_url = f"https://api.bittrex.com/v3/markets/{symbol}/ticker"
        self.symbol = symbol
    ...
    def fetch_price(self):
        try:
            ...
            return float(response.json()['lastTradeRate'])  # Last trade price
    ...
    def start(self):
        logging.info(f"Starting BittrexBot for {self.symbol}")
    ...
    def stop(self):
        self.stop_event.set()
        self.thread.join()
        logging.info("BittrexBot stopped.")
...
if __name__ == "__main__":
    ...
    bittrex_bot = BittrexBot(symbol, reporting_bot_url, email_config)
    try:
        bittrex_bot.start()
