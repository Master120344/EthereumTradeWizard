...
class BitfinexBot:
    def __init__(self, symbol, reporting_bot_url, email_config, check_interval=5):
        self.api_url = f"https://api-pub.bitfinex.com/v2/tickers?symbols={symbol}"
        self.symbol = symbol
    ...
    def fetch_price(self):
        try:
            ...
            return float(response.json()[0][6])  # Last price from the response
    ...
    def start(self):
        logging.info(f"Starting BitfinexBot for {self.symbol}")
    ...
    def stop(self):
        self.stop_event.set()
        self.thread.join()
        logging.info("BitfinexBot stopped.")
...
if __name__ == "__main__":
    ...
    symbol = "tBTCUSD"  # Example trading pair; adjust as needed

    bitfinex_bot = BitfinexBot(symbol, reporting_bot_url, email_config)
    try:
        bitfinex_bot.start()
