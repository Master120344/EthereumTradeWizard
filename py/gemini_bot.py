...
class GeminiBot:
    def __init__(self, symbol, reporting_bot_url, email_config, check_interval=5):
        self.api_url = f"https://api.gemini.com/v1/pubticker/{symbol}"
        self.symbol = symbol
    ...
    def fetch_price(self):
        try:
            ...
            return float(response.json()['last'])  # Last price from the response
    ...
    def start(self):
        logging.info(f"Starting GeminiBot for {self.symbol}")
    ...
    def stop(self):
        self.stop_event.set()
        self.thread.join()
        logging.info("GeminiBot stopped.")
...
if __name__ == "__main__":
    ...
    symbol = "btcusd"  # Example trading pair; adjust as needed

    gemini_bot = GeminiBot(symbol, reporting_bot_url, email_config)
    try:
        gemini_bot.start()
